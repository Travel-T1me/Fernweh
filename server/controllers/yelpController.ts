...
export const getRestaurants = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  if (!location) {
    ...
    const apiUrl = `https://yelp-reviews.p.rapidapi.com/business-search?query=Restaurants&location=${encodeURIComponent(location)}`;
    const headers = {
...
// config options for axios request
//   const options = {
//     method: 'GET',
